      ...
      <div className="flex flex-wrap justify-between items-center gap-4">
        <div>
          <h2 className="page-title flex items-center gap-3">
            <Swords className="w-8 h-8 lg:w-9 lg:h-9 text-fut-emerald shrink-0" />
            {t('compare')}
          </h2>
                ...
                {(['PAC', 'SHO', 'PAS', 'DRI', 'DEF', 'PHY'] as const).map(attr => {
                  ...
                  return (
                    <tr key={attr} className="border-b border-white/5">
                      <td className="py-2.5 font-stats font-bold text-fut-gold">{attr}</td>
                      <td className={`py-2.5 text-right font-stats ${winA ? 'text-fut-emerald font-bold' : ''}`}>{a}</td>
                      <td className={`py-2.5 text-right font-stats ${winB ? 'text-fut-emerald font-bold' : ''}`}>{b}</td>
                    </tr>
                  )
                })}
              </tbody>
